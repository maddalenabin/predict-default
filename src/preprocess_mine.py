...
class KlarnaDataProcessor:
    ...
    def feature_engineering(self):
        ...
        if all(col in self.df.columns for col in ['num_active_loans', 'existing_klarna_debt']):
            self.df['high_risk_customer'] = ((self.df['num_active_loans'] >= 3) & # 3 is set arbitrarily, more studies needed to set something meaningful
                                (self.df['existing_klarna_debt'] > self.df['existing_klarna_debt'].median())
                                ).astype(int)
            self.engineered_features.append('high_risk_customer')
        """
        ...
        # 8. remove new_exposure_7d, as it didnt' show correlation with target
        #if "new_exposure_7d" in self.df.columns:
        #    self.df = self.df.drop(columns=['new_exposure_7d'])
